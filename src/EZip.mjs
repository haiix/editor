...
export const passwordPrompt = TDialog.create(
...
export default class EZip {
  ...
  async save(callback) {
    const name = this.setting.fileName.endsWith('.zip')
      ? this.setting.fileName.slice(0, -4)
    ...
    const zipFileName =
      ...
      (formValues.name.endsWith(EXT) ? '' : EXT);
    const zipFilePassword = formValues.password;

    const inputFiles = await callback();

    const zipBlob = await this.createEncryptedZipBlob(
      zipFilePassword,
      inputFiles,
    );
  ...
  async createEncryptedZipBlob(password, inputFiles) {
    ...
    return password
      ? await this.createZip([{ path: 'encrypted.zip', file: innerZipBlob }], {
          password,
          level: 5,
        })
      : innerZipBlob;
  ...
  async readEncryptedZipFile(zipFile) {
    try {
      ...
      return files.length === 1 && files[0].path === 'encrypted.zip'
